...
const MoviesExplore = () => {
  ...
  const defineQueryString = (page: number): string => {
    ...
    return `${BASE_URL}/discover/movie?page=${page}${genres}${order}`;
  };
  ...
  return (
    <section className="explore-movie">
      <ContentWrapper>
        <div className="explore-movie__header">
          <h2 className="explore-movie__title">Explore Movies</h2>
          <div className="explore-movie__filters">
            <Filters genres={genres?.genres} handler={handleFilters} />
          </div>
        </div>
        {error && (
          <div className="explore-movie__info">
            <h2>{error}</h2>
        ...
        >
          <div className="explore-movie__list">
            {data?.results &&
